...
class pqp_pi
{
  ...
  static function gen_pqp_db_results()
  {
    ...
    if (in_array('adodb', array_keys(get_object_vars($CI))) AND is_object($CI->adodb)) {
      //get data from adodb
      $db_obj->queryCount += count($CI->adodb->queries);
      foreach ($CI->adodb->queries as $key => $val)
      {
        $time = number_format($CI->adodb->query_times[$key], 4);
