...
echo '<div id="pqp-container" class="pQp hideDetails" style="display:none;z-index:999;">';
		...
		foreach($output['queries'] as $query) {
			...
			if(isset($query['explain'])) {
				foreach( $query['explain'] AS $row) {
					echo '<em>
						Select Type: <b>'.$row['select_type'].'</b> &middot; 
						Table: <b>'.$row['table'].'</b> &middot; 
						Type: <b>'.$row['type'].'</b> &middot; 
						Possible keys: <b>'.$row['possible_keys'].'</b> &middot; 
						Key Used: <b>'.$row['key'].'</b> &middot; 
						Rows: <b>'.$row['rows'].'</b>
					</em>';
				}
				echo '<em>
